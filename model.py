...
class ConvCore(nn.Module):
    def __init__(self, dropout=None, depth_scale=1, modality='cough'):
        super().__init__()
        self.d = depth_scale
        self.dropout = nn.Dropout(p=0.5) if dropout else None
        self.conv1 = nn.Conv2d(3 if modality == 'speechbreath' else 1,
                               int(32 * self.d),
                               kernel_size=(7, 3),
                               padding=1)
...
class Conv_Model(ConvCore):
    def __init__(self, dropout=None, depth_scale=1, device="cuda", input_shape=(1025, 94), modality='cough'):
        super().__init__(dropout, depth_scale, modality=modality)
        self.to(device)
        out_tmp = super().forward(
            torch.randn(1, 3 if modality == 'speechbreath' else 1, *input_shape).to(device))
